"""
Fixed large-scale population script for ARGO data.
Generates 1500 profiles with proper measurements - simplified and fixed.
"""
import sys
import os
import pandas as pd
...
def create_large_oceanographic_data():
    ...
    for i, float_data in enumerate(sample_floats):
        ...
        for cycle in range(1, 151):
            ...
            # Each profile has 50 measurements (to get ~75,000 total measurements)
            num_levels = random.randint(45, 55)
            ...
            depths = np.logspace(np.log10(min_depth), np.log10(max_depth), num_levels)
            ...
            for j, depth in enumerate(depths):
                ...
                measurement = {
                    ...
                    'nitrate': nitrate,
                    'ph': ph,
                    'temperature_qc': random.choice([1, 2] if random.random() < 0.96 else [2]),
                    'salinity_qc': random.choice([1, 2] if random.random() < 0.94 else [2]),
                    'oxygen_qc': random.choice([1, 2, 3]),
                    'chlorophyll_qc': random.choice([1, 2, 3]),
                    'nitrate_qc': random.choice([1, 2, 3]),
                    'ph_qc': random.choice([1, 2])
                }
                sample_measurements.append(measurement)
            
            # Progress reporting
            if profile_id % 100 == 0:
                logger.info(f"Generated {profile_id}/1500 profiles with {len(sample_measurements)} measurements so far...")
...
def populate_large_database():
    ...
    try:
        ...
        floats, profiles, measurements, trajectories = create_large_oceanographic_data()
        ...
        for i, profile in enumerate(profiles):
            try:
                ...
                if (i + 1) % 150 == 0:
                    logger.info(f"Stored {i + 1}/1500 profiles...")
            except Exception as e:
                logger.error(f"Error storing profile {profile['float_id']}-{profile['cycle_number']}: {e}")

        # Store measurements in optimized batches
