...
class DatabaseManager:
    ...
    def store_measurements(self, measurements: List[Dict[str, Any]]) -> List[str]:
        """Store ARGO measurement data in bulk with enhanced NetCDF variables."""
        session = self.get_session()
        measurement_ids = []
        try:
            for measurement in measurements:
                measurement_record = ARGOMeasurement(
                    ...
                    pressure_adjusted=measurement.get('pressure_adjusted'),
                    temperature_adjusted=measurement.get('temperature_adjusted'),
                    salinity_adjusted=measurement.get('salinity_adjusted'),
                    
                    # Error estimates
                    pressure_adjusted_error=measurement.get('pressure_adjusted_error'),
                    temperature_adjusted_error=measurement.get('temperature_adjusted_error'),
                    salinity_adjusted_error=measurement.get('salinity_adjusted_error'),
                    
                    # QC flags
                    pressure_qc=measurement.get('pressure_qc'),
                    temperature_qc=measurement.get('temperature_qc'),
                    salinity_qc=measurement.get('salinity_qc'),
                    pressure_adjusted_qc=measurement.get('pressure_adjusted_qc'),
                    temperature_adjusted_qc=measurement.get('temperature_adjusted_qc'),
                    salinity_adjusted_qc=measurement.get('salinity_adjusted_qc'),
    ...
    def get_measurements_for_analysis(self, parameter: str = 'temperature', 
                                     min_lat: float = None, max_lat: float = None,
                                     min_lon: float = None, max_lon: float = None,
                                     start_date: datetime = None, end_date: datetime = None,
                                     limit: int = 1000) -> pd.DataFrame:
        """Get measurement data for analysis and time series plotting."""
        session = self.get_session()
        try:
            # Build dynamic query based on available constraints
            where_conditions = []
            ...
            logger.error(f"Error getting measurements for analysis: {e}")
            return pd.DataFrame()
        finally:
            session.close()
